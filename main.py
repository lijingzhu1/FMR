...
from src.model.GKGEL import TransE as GKGEL
...
class experiment():
    ...
    def create_model(self):
        ...
        elif self.args.lifelong_name == 'GKGEL':
            model = GKGEL(self.args, self.kg)
        elif self.args.lifelong_name == 'base_model':
            model = base_model(self.args, self.kg)
    ...
    def reset_model(self, model=False, optimizer=False):
        ...
        if model:
            ...
            if self.args.lifelong_name == 'GEM':
                self.args.optimizer = self.optimizer
        if optimizer:
            if self.args.lifelong_name == 'GKGEL' and self.args.dataset == 'ENTITY' and self.args.snapshot == 2: 
                # print(f'111gogogo')
                self.optimizer = torch.optim.Adam(self.model.parameters(), lr=float(self.args.learning_rate), weight_decay=self.args.l2)
            elif self.args.lifelong_name == 'GKGEL' and self.args.dataset == 'ENTITY' and self.args.snapshot == 3: 
                # print(f'222gogogo')
                self.optimizer = torch.optim.Adam(self.model.parameters(), lr=float(self.args.learning_rate), weight_decay=1e-5)
            else:
                self.optimizer = torch.optim.Adam(self.model.parameters(), lr=float(self.args.learning_rate), weight_decay=self.args.l2)
            if self.args.lifelong_name == 'GEM':
                self.args.optimizer = self.optimizer
    ...
    def train(self):
        ...
        for epoch in range(int(self.args.epoch_num)):
            ...
            '''training'''
            loss, valid_res,new_loss,MAE_loss,regular_loss = trainer.run_epoch()
            '''early stop'''
            ...
            '''logging'''
            if epoch % 1 == 0:
                self.args.logger.info('Snapshot:{}\tEpoch:{}\ttotal_Loss:{}\tnew_Loss:{}\tMAE_Loss:{}\tReguar_Loss:{}\tMRR:{}\tMR:{}\tHits@10:{}\tBest:{}'.format(self.args.snapshot, epoch,round(loss, 3),round(new_loss, 3),round(MAE_loss, 3),round(regular_loss,3), round(valid_res['mrr'] * 100, 2),round(valid_res['mr'], 2), round(valid_res['hits10'] * 100, 2), round(self.best_valid*100,2)))
        end_time = time.time()
