...
from torch_geometric.nn import GCN,GraphSAGE,GAT,EdgeCNN
import copy
...
class LKGE(BaseModel):
    ...
    def store_old_parameters(self):
        ...
        for param in self.old_model.parameters():  # Disable gradient computation for the old model
    ...
    def switch_snapshot(self):
        ...
        if self.args.using_embedding_transfer == 'True':
            ...
            # Apply Procrustes alignment to old entities and relations
            # aligned_old_ent_embeddings = self.procrustes(new_ent_embeddings[:self.kg.snapshots[self.args.snapshot].num_ent],reconstruct_ent_embeddings[:self.kg.snapshots[self.args.snapshot].num_ent])
            # aligned_old_rel_embeddings = self.procrustes(new_rel_embeddings[:self.kg.snapshots[self.args.snapshot].num_rel],reconstruct_rel_embeddings[:self.kg.snapshots[self.args.snapshot].num_rel])
            # print(f'aligned_old_ent_embeddings,aligned_old_rel_embeddings:',aligned_old_ent_embeddings.size(),aligned_old_rel_embeddings.size())
            # new_ent_embeddings[:self.kg.snapshots[self.args.snapshot].num_ent] = aligned_old_ent_embeddings
            # new_rel_embeddings[:self.kg.snapshots[self.args.snapshot].num_rel] = aligned_old_rel_embeddings
    ...
    def compute_distillation_loss(self, new_output, old_output):
        """
        Computes the distillation loss between the new model output and the old model output.
        """
        distillation_loss = F.kl_div(F.log_softmax(new_output / self.args.temperature, dim=1),
                                     F.softmax(old_output / self.args.temperature, dim=1),
                                     reduction='batchmean') * (self.args.temperature ** 2)
...
class TransE(LKGE):
    ...
    def split_batch(self, head, rel, batch_size):
        """
        Splits the batch into smaller sub-batches.
        """
        num_batches = (head.size(0) + batch_size - 1) // batch_size
        for i in range(num_batches):
            yield head[i * batch_size:(i + 1) * batch_size], rel[i * batch_size:(i + 1) * batch_size]
    ...
    def loss(self, head, rel, tail=None, label=None):
        ...
        if self.old_model is not None:
            head_mask = head < self.old_model.ent_embeddings.weight.size(0)
            relation_mask = rel < self.old_model.rel_embeddings.weight.size(0)
            mask = head_mask & relation_mask
            filtered_heads = head[mask]
            filtered_relations = rel[mask]
            if len(filtered_heads) > 0:
                distillation_losses = []
                batch_size = 50 # Use a smaller batch size for prediction
                for sub_heads, sub_rels in self.split_batch(filtered_heads, filtered_relations, batch_size):
                    with torch.no_grad():
                        old_output = self.old_model.predict(sub_heads, sub_rels)
                    new_output = self.predict(sub_heads, sub_rels)
                    # Create mask for valid outputs
                    new_output = new_output[:, :self.old_model.ent_embeddings.weight.size(0)]


                    distillation_loss = self.compute_distillation_loss(new_output, old_output)
                    distillation_losses.append(distillation_loss)

                total_distillation_loss = torch.stack(distillation_losses).mean()
                loss += float(self.args.lwf_weight) * total_distillation_loss
        return loss
...
class ConvLayer(nn.Module):
    ...
    def forward(self, x, r, edge_index, edge_type, num_ent, num_rel, old_entity_weight, old_relation_weight, old_x, old_r):
        ...
        if old_entity_weight == None:  # for embedding transfer
            ...
            ent_embed = scatter_mean(src=neigh_h + neigh_r, dim=0, index=edge_index[1].long(), dim_size=num_ent)
            rel_embed = scatter_mean(src=neigh_t - neigh_h, dim=0, index=edge_type.long(), dim_size=num_rel + 1)
            ent_embed = self.GCN_model(ent_embed.float(), edge_index, edge_attr = edge_type)
