...
class TrainBatchProcessor():
    ...
    def process_epoch(self, model, optimizer):
        ...
        for idx_b, batch in enumerate(self.data_loader):
            ...
            bh, br, bt, by = batch
            optimizer.zero_grad()
            batch_loss,new_loss,MAE_loss,regular_loss = model.loss(bh.to(self.args.device),
            ...
            new_losses += new_loss.item()
            MAE_losses += MAE_loss.item()
            regular_losses += regular_loss.item()
            '''post processing'''
            model.epoch_post_processing(bh.size(0))
        return total_loss,new_losses,MAE_losses,regular_losses
